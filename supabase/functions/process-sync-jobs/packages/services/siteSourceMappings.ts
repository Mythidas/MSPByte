'use server';

import { tables } from '../db/index.ts';
import { TablesInsert, TablesUpdate } from '../db/schema.ts';

export function getSiteMappings(sourceId?: string) {
  return tables.select('site_mappings_view', (query) => {
    query = query.eq('is_parent', false).order('site_name').order('parent_name');
    if (sourceId) {
      query = query.eq('source_id', sourceId);
    }
  });
}

export function getSiteMapping(id: string) {
  return tables.selectSingle('site_mappings_view', (query) => {
    query = query.eq('id', id);
  });
}

export function getSiteSourceMapping(sourceId: string, siteId: string) {
  return tables.selectSingle('site_source_mappings', (query) => {
    query = query.eq('source_id', sourceId).eq('site_id', siteId);
  });
}

export function getSiteSourceMappings(sourceId?: string, siteIds?: string[]) {
  return tables.select('site_source_mappings', (query) => {
    if (sourceId) query = query.eq('source_id', sourceId);
    if (siteIds) query = query.in('site_id', siteIds);
  });
}

export function putSiteSourceMapping(mapping: TablesInsert<'site_source_mappings'>[]) {
  return tables.insert('site_source_mappings', mapping);
}

export function updateSiteSourceMapping(id: string, mapping: TablesUpdate<'site_source_mappings'>) {
  return tables.update('site_source_mappings', id, mapping);
}

export function deleteSiteSourceMapping(id: string) {
  return tables.delete('site_source_mappings', [id]);
}
