...
export const registerAction = async (_prevState: any, params: FormData) => {
  ...
  const { data: invite } = await supabase.from('invites').select().eq('id', valid.data.code).single();
  ...
  if (error) {
    ...
    if (data.user) {
      ...
      const { data: user } = await supabaseAdmin.from("users").select().eq("id", data.user.id).single();
